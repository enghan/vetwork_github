...
Widget service_Card(List<exist_address> Service, int position) {
  ...
  return Card(
      ...
      child: InkWell(
          onTap: () {
            var price = double.parse('${Service[position].phone}');
...
Widget build_cardaddress() {
  return Container(
      ...
      child:   ListView.builder(itemCount: address.length,scrollDirection: Axis.vertical,
          itemBuilder: (context,int  position) {return
            service_Card(address, position);
          })
  );
}
